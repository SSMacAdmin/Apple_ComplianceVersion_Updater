<#
.SYNOPSIS
    Diagnostics script for the macOS Intune Compliance Policy runbook.
...
.DESCRIPTION
    ...
    4. Test access to the macOS compliance policy
    5. Test SOFA macOS feed access
...
try {
    ...
    if ($useManagedIdentity) {
        ...
        $policyId = Get-AutomationVariable -Name "MACOS_POLICY_ID"
        Write-Output "  MACOS_POLICY_ID: $($policyId.Substring(0,8))..."
    }
    else {
    ...
    Write-Output ""
    Write-Output "  FIX:"
    Write-Output "  - Check that MACOS_POLICY_ID and USE_MANAGED_IDENTITY exist in Automation Variables"
    Write-Output "  - For Service Principal: also ensure INTUNE_TENANT_ID, INTUNE_CLIENT_ID, INTUNE_CLIENT_SECRET exist"
...
try {
    ...
    if ($useManagedIdentity) {
        ...
        Write-Output "  - See SETUP-GUIDE.md for the permission grant command"
    }
    else {
        Write-Output ""
...
try {
    ...
    foreach ($p in $policies.value) {
        $marker = if ($p.id -eq $policyId) { " <-- TARGET (macOS)" } else { "" }
...
# ─────────────────────────────────────────────────────────────────────────────
Write-Output "[STEP 4] Testing Access to Target macOS Policy..."
try {
    $headers = @{
        "Authorization" = "Bearer $accessToken"
        "Content-Type"  = "application/json"
    ...
    Write-Output "  Is macOS policy:   $isMacOSPolicy"
...
# ─────────────────────────────────────────────────────────────────────────────
# STEP 5: SOFA macOS Feed
# ─────────────────────────────────────────────────────────────────────────────
Write-Output "[STEP 5] Testing SOFA macOS Feed Access..."
try {
    ...
    if ($null -eq $sofaData -or $null -eq $sofaData.OSVersions) {
        Write-Output "  FAIL: SOFA returned no OS versions"
    }
    else {
        $totalVersions = 0
        ...
        foreach ($mv in $sofaData.OSVersions | Select-Object -First 4) {
            if ($mv.Latest) {
                Write-Output "  - macOS $($mv.Latest.ProductVersion) (Build: $($mv.Latest.Build), Released: $($mv.Latest.ReleaseDate))"
            }
        }
        Write-Output ""
        Write-Output "  RESULT: SOFA macOS feed accessible"
    }
...
Write-Output "========================================="
Write-Output ""
Write-Output "If all steps passed, the macOS runbook is ready."
Write-Output "Run Update-IntuneMacOSCompliance as a test to verify end-to-end."
Write-Output ""
